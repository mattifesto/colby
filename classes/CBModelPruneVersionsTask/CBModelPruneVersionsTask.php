...
CBModelPruneVersionsTask {
    ...
    CBTasks2_run(
        ...
        try {
            $targetModelCBIDAsSQL = CBID::toSQL(
                $targetModelCBID
            );
            ...
            foreach (
                ...
                if (
                    ...
                    $replacedAsSQL = CBConvert::valueAsInt(
                        $version->replaced
                    );

                    $versionAsSQL = CBConvert::valueAsInt(
                        $version->version
                    );
                    ...
                    $SQL = <<<EOT
                        ...
                        WHERE   ID = {$targetModelCBIDAsSQL} AND
            ...
            if (
                !empty($versionsToPrune)
            ) {
                $versionsToPruneAsSQL = implode(
                    ',',
                    $versionsToPrune
                );
            ...
            throw new CBExceptionWithValue(
                'An exception was thrown during CBTasks2_run()',
                (object)[
                    'targetModelCBID' => $targetModelCBID,
                ],
                'e657df4ab982bca27be8b17bd39578db731d68c5',
                0,
                $throwable
            );
        }
    ...
    assignActions(
        ...
        for (
            ...
            if (
                ...
                throw new CBExceptionWithValue(
                    'The lifespan of a version is negative.',
                    $versions,
                    '0eff9c8cb5df64adfed9d0a2b6811f7944eb0efb'
                );
            }
            ...
            if ($deathspan < 0) {
                throw new CBExceptionWithValue(
                    'The deathspan of a version is negative.',
                    $versions,
                    '72a78268a09c6db25c26a6f19ba7af9e91c04289'
                );
            }

            if (
                $deathspan < 1000
            ) { /* 0 to 16.6 minutes: 10 seconds */
                if (
                    $lifespan < 10
                ) {
                    $version->action = 'prune';
                ...
                $deathspan < 100000
            ) { /* 16.6 minutes to 1.1 days: 1.6 minutes */
                if (
                    $lifespan < 100
                ) {
                    $version->action = 'prune';
                ...
                $deathspan < 1000000
            ) { /* 1.1 days to 11.5 days: 16.6 minutes */
                if (
                    $lifespan < 1000
                ) {
                    $version->action = 'prune';
                ...
                $deathspan < 10000000
            ) { /* 11.5 days to 3.8 months: 2.7 hours */
                if (
                    $lifespan < 10000
                ) {
                    $version->action = 'prune';
